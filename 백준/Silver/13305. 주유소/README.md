...
### 출력 
 ...
 <p>주유는 가장 저렴한 곳에서 해야한다. 처음 주유소에서는 일단 다음 도시까지라도 갈 기름을 넣어야 한다. 따라서 첫 번째 도시의 주유소 가격을 최저가로 설정하고 이동 비용을 계산한다. 그 후에 각 도시를 순회할 때마다 해당 도시의 주유소 가격을 확인한다. 만약 현재 주유소가 기존 최저가 보다 저렴하면 최저가를 현재 주유소 가격으로 변경한다. 이렇게 구간별 최저가에 다음 도시까지의 거리를 곱해서 최종 비용을 출력한다.</p>

 ###답안의 수정사항

<p>처음 답안을 제출 했을 때, 58점이 나왔었다. 문제의 입력 조건을 다시 확인했을 때, 총 거리가 1 이상 1,000,000,000이하의 자연수 이고 리터당 가격도 1이상 1,000,000,000이하의 자연수임을 확인했다. 이런 값들이 곱연산 될 경우 int형의 범위를 벗어난다. 따라서 기존에 int형으로 선언했던 변수들을 long long형으로 수정하여 다시 제출 하여서 100점 확인을 했다.</p>
